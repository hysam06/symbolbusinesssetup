...
const LandingAnimation = () => {
    ...
    // Determine visibility: only show if on the homepage ('/')
    const [isVisible, setIsVisible] = useState(() => {
        return location.pathname === '/';
    });

    const containerRef = useRef(null);
    const pathsRef = useRef([]); // Refs for the hidden SVG paths
    const dotsRef = useRef([]); // Refs for the visible Dots

    useLayoutEffect(() => {
        // If not ensuring home page or not visible, do nothing
        if (!isVisible) return;

        // Ensure we have refs and element is in DOM
        ...
        let ctx = gsap.context(() => {
            ...
            dotElements.forEach((dot, i) => {
                if (pathElements[i] && dot) {
                    ...
                    // Distribute points evenly on a sphere approximation
                    ...
                    // Set start pos (Globe)
                    gsap.set(dot, {
                        attr: { cx: globeX, cy: globeY, r: 2 }, // Start smaller
            ...
            }, "+=0.2"); // Subtle pause after globe

            // Phase 3: Dot Alignment into Logo (1.4s - 2.3s)
            // Magnetic snap to final positions
            tl.to(dotElements, {
                attr: { cx: (i, t) => t.dataset.finalX, cy: (i, t) => t.dataset.finalY, r: 4 },
                duration: 0.9,
                ease: "back.out(1.2)", // Subtle bounce for "magnetic" feel
            ...
            // Fade in paths (Real Logo) without Glow
        ...
        return () => ctx.revert();
    }, [isVisible]); // Re-run if isVisible changes, though it mainly starts true then goes false
    ...
    return (
        <div ref={containerRef} style={{
            ...
            backgroundColor: '#ffffff', zIndex: 9999, display: 'flex', justifyContent: 'center', alignItems: 'center'
        }}>
